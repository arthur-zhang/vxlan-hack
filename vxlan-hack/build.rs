...
/// This crate has a runtime dependency on artifacts produced by the `vxlan-hack-ebpf` crate.
...
/// crate's undeclared dependency on artifacts from `vxlan-hack-ebpf` would cause build (and
...
/// performance; an environment variable is used to replace true builds of `vxlan-hack-ebpf`
/// with stubs to preserve the property that code generation and linking (in
/// `vxlan-hack-ebpf`) do not occur on metadata-only actions such as `cargo check` or `cargo
...
fn main() {
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `vxlan-hack-ebpf`, so cargo will automatically rebuild us
        // if `vxlan-hack-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `vxlan-hack-ebpf`'s *binary* targets, that only gets us half of the way. This
